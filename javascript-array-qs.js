...
// 11. Duplicate Numbers: Given an array of integers, write a function that finds all the duplicate numbers.
...
// 12. Merge Intervals: Implement a function to merge overlapping intervals given as an array of pairs.
...
function mergeIntervals(arr) {
  ...
  for (let i = 1; i < arr.length; i++) {
    if (arr[i][0] <= result[result.length - 1][1]) {
      result[result.length - 1][1] = Math.max(arr[i][1], result[result.length - 1][1]);
    } else {
      result.push(arr[i]);
    }
  }
...
// Optimised
function mergeIntervalsOptimized(arr) {
  arr.sort((a, b) => a[0] - b[0]);
  let result = [arr[0]];
...
// Rotate Array: Write a function that rotates an array to the right by a certain number of steps.
